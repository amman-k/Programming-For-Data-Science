...
BranchName <- c(
  "Vellore Town","Chennai Central","Mumbai Andheri",
  "Pune Hinjewadi","Delhi Karol Bagh",
  "Noida Sector 18","Kolkata Park St",
...
Segment <- c(
  "Grocery","Electronics","Grocery","Home","Apparel",
...
# Sort by rank
branches_by_rank <- branches_in[
  order(branches_in$Rank, branches_in$BranchID),
]

# Write ranked CSV
write.csv(
  branches_by_rank,
  file = branches_rank_csv,
  row.names = FALSE,
  na = ""
)

cat("Branches by rank CSV:",
    normalizePath(branches_rank_csv, winslash = "/"), "\n")
...
write.csv(
  quarter_avg_df,
  file = quarter_avg_csv,
  row.names = FALSE,
  na = ""
)

cat("Quarter-wise averages CSV:",
    normalizePath(quarter_avg_csv, winslash = "/"), "\n")
...
region_quarter_avg$Overall_Average <- rowMeans(
  region_quarter_avg[, quarter_cols],
  na.rm = TRUE
)

# Sort by overall average
region_quarter_avg <- region_quarter_avg[
  order(-region_quarter_avg$Overall_Average,
        region_quarter_avg$Region),
]
...
cat("Region-wise averages CSV:",
    normalizePath(region_avg_csv, winslash = "/"), "\n")
